...
export const getCategoryById = async (id: CategoryId) => {
  ...
  const [row] = await db.select().from(categories).where(and(eq(categories.id, categoryId), eq(categories.userId, session?.user.id!)));
...
export const getCategoryByIdWithTransactions = async (id: CategoryId) => {
  const { session } = await getUserAuth();
  const { id: categoryId } = categoryIdSchema.parse({ id });
  const rows = await db.select({ category: categories, transaction: transactions }).from(categories).where(and(eq(categories.id, categoryId), eq(categories.userId, session?.user.id!))).leftJoin(transactions, eq(categories.id, transactions.categoryId));
  if (rows.length === 0) return {};
  const c = rows[0].category;
  const ct = rows.filter((r) => r.transaction !== null).map((t) => t.transaction) as CompleteTransaction[];
